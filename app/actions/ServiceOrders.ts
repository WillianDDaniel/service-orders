...
export async function createServiceOrder(formData: FormData) {
  ...
  await sql`
    INSERT INTO public.service_orders (name, price, description, status, tag)
    VALUES (${name}, ${price}, ${description || null}, ${status}::service_order_status, ${tag}::service_order_tag)
  `;
